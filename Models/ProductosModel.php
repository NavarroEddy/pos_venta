...
class ProductosModel extends Query{
    private $codigo, $nombre, $precio_compra, $id_medida, $id_categoria, $precio_venta, $img;
    ...
    public function registrarProducto(string $codigo, string $nombre, string $precio_compra, string $precio_venta, int $id_medida, int $id_categoria, string $img)
    {
        ...
        $verficar = "SELECT * FROM Productos WHERE codigo = '$this->codigo'";
        $existe = $this->select($verficar);
        if (empty($existe)) {
            $sql = "INSERT INTO productos(codigo,descripcion,precio_compra,precio_venta,id_medida,id_categoria,foto) value (?,?,?,?,?,?,?)";
