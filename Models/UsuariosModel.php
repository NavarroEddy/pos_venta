...
class UsuariosModel extends Query
{
    ...
    {
        $sql = "SELECT u.*,c.id as id_caja,c.caja,u.estado FROM usuarios u INNER JOIN  caja c WHERE u.id_caja = c.id";
    ...
    public function registrarUsuario(string $usuario, string $nombre, string $clave, int $id_caja)
    {
        ...
        $verficar = "SELECT * FROM usuarios WHERE usuario = '$this->usuario'";
        $existe = $this->select($verficar);
        if (empty($existe)) {
            $sql = "INSERT INTO usuarios(usuario,nombre,clave,id_caja) value (?,?,?,?)";
