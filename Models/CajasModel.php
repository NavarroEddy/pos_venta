...
class CajasModel extends Query{
    ...
    public function registrarCaja(string $caja)
    {
        ...
        if (empty($existe)) {
            $sql = "INSERT INTO caja (caja) value (?)";
    ...
    public function registrarArqueo(int $id_usuario, string $monto_inicial,  string $fecha_apertura)
    {
        ...
        $existe = $this->select($verficar);
        if (empty($existe)) {
            $sql = "INSERT INTO cierre_caja (id_usuario, monto_inicial, fecha_apertura) value (?,?,?)";
    ...
    {
        $sql = "SELECT id,monto_inicial,estado FROM cierre_caja WHERE id_usuario = $id_user AND estado =1 ";
        $data = $this->select($sql);
        return $data;
    }
